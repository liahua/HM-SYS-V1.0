...
public class SysMenuServiceImpl implements SysMenuService {
	...
	public int deleteObject(Integer id) {
		//验证
		if(id==null||id<0)
		throw new ClassExceptionTest("请选择需要删除的对象");
		int cCount = sysMenusMapper.getChildCount(id);
		if(cCount>0)
		throw new ClassExceptionTest("请先删除子菜单");
		int row = sysMenusMapper.deleteByPrimaryKey(id);
		if(row==0)
		throw new ClassExceptionTest("该选项可能已经不存在了");
		//删除菜单关系数据
	...
	public int insert(SysMenus record) {
		//验证
		if(record==null)
			throw new ClassExceptionTest("修改对象不能为空");
		if(StringUtils.isEmpty(record.getName()))
			throw new ClassExceptionTest("菜单名不能为空");
		int row;
		try {
	...
	public int update(SysMenus record) {
		if(record==null)
			throw new ClassExceptionTest("修改选项不能为空");
		if(StringUtils.isEmpty(record.getName()))
			throw new ClassExceptionTest("菜单名不能为空");
